...
export default function Dashboard() {
  ...
  const { businessType: branchBusinessType, businessSubType: branchBusinessSubType } =
    useBranchBusiness();
  ...
  const {
    ...
    const todayRefundTotal = todayRefundedSales.reduce(
      (acc: number, s: any) => acc + parseNumeric(s.total),
      0,
    );
    const todayRefundCount = todayRefundedSales.length;
    const totalGrossRevenue = todaySales.reduce(
      (acc: number, s: any) => acc + parseNumeric(s.total),
      0,
    );
    ...
    for (const sale of todaySales) {
      ...
      for (const item of items) {
        ...
        const mods = (item.modifiers ?? []).reduce(
          (s: number, m: any) => s + parseNumeric(m.price),
          0,
        );
  ...
  const reportHighlights = [
    ...
    },
    {
      ...
      value: formatCurrency(todayRefundTotal, currency),
      icon: Percent,
    },
    {
      label: t("dashboard.peakHour"),
      value: todaySales.length
        ? `${bestSeller ? bestSeller.qty : 0} ${t("dashboard.sold")}`
        : t("dashboard.noSalesToday"),
      icon: Clock3,
    },
    {
      label: t("dashboard.payments"),
      value: paymentBreakdown.length ? paymentBreakdown[0].method : t("dashboard.noPaymentData"),
      icon: PieChart,
    },
  ];

  const CurrencyIcon = useCallback(
    ({ className: _className }: { className?: string }) => (
  ...
  return (
    <div className="space-y-4 page-enter">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          <p className="text-sm text-muted-foreground">
            {todaySales.length}{" "}
            {todaySales.length === 1 ? terminology.orderLabel : `${terminology.orderLabel}s`}{" "}
      ...
      <div
        data-tour="tour-dashboard-kpi"
        className="grid gap-3 grid-cols-2 lg:grid-cols-4 stagger-children"
      >
        {[
          {
          ...
          {
            label: t("dashboard.taxCollected"),
          ...
          <div
            key={i}
            className={`glass-card glow-hover-card rounded-2xl p-4 bg-gradient-to-br ${card.glow} to-transparent animate-fade-scale card-press`}
          >
            ...
            <p
              className={`text-xl font-bold tabular-nums truncate ${card.color}`}
              title={card.display}
            >
              {card.display}
            </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger-reveal">
          <div className="rounded-2xl border border-border/30 bg-secondary/20 p-3 mini-stat-card">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
              {t("dashboard.grossSales")}
            </p>
            <p className="text-sm font-bold tabular-nums">
              {formatCurrency(daySummary.gross, currency)}
            </p>
          </div>
          <div className="rounded-2xl border border-border/30 bg-secondary/20 p-3 mini-stat-card">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
            ...
            <p className="text-sm font-bold tabular-nums">
              {formatCurrency(daySummary.net, currency)}
            </p>
          </div>
          <div className="rounded-2xl border border-border/30 bg-secondary/20 p-3 mini-stat-card">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
              {t("dashboard.vatCollected")}
            </p>
            <p className="text-sm font-bold tabular-nums">
              {formatCurrency(daySummary.tax, currency)}
            </p>
          </div>
          <div className="rounded-2xl border border-border/30 bg-secondary/20 p-3 mini-stat-card">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">
            ...
            <p className="text-sm font-bold tabular-nums">
              -{formatCurrency(daySummary.refunds, currency)}
            </p>
          </div>
        </div>
      </div>
              ...
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 row-interactive">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {p.sku ? `SKU: ${p.sku} · ` : ""}
                    {t("dashboard.threshold")}: {p.lowStockThreshold}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p
                    className={[
                      ...
                      (p.stock ?? 0) === 0 ? "text-rose-500" : "text-amber-600 dark:text-amber-400",
                    ].join(" ")}
                  >
                    {p.stock ?? 0}
                  </p>
      ...
      <div className="grid grid-cols-2 gap-3 stagger-reveal">
          ...
          {bestSeller ? (
            <>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {bestSeller.qty} {terminology.itemUnit}
                {bestSeller.qty !== 1 ? "s" : ""} ·{" "}
                <span className="text-primary font-semibold tabular-nums">
                  {formatCurrency(bestSeller.revenue, currency)}
                </span>
              </p>
            </>
          ) : (
        ...
        <div className="glass-card glow-hover-card rounded-2xl p-4 bg-gradient-to-br from-primary/8 to-transparent animate-fade-scale">
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
              {t("dashboard.allTimePerformance")}
            </p>
          </div>
          <p className="font-bold text-sm tabular-nums">{formatCurrency(allTime.net, currency)}</p>
          <p className="text-xs text-muted-foreground mt-1">
            {allTime.orderCount}{" "}
            {allTime.orderCount === 1 ? terminology.orderLabel : `${terminology.orderLabel}s`} total
          </p>
          {allTime.refundTotal > 0 && (
            <p className="text-[10px] text-rose-500 mt-0.5">
              -{formatCurrency(allTime.refundTotal, currency)} refunded
            </p>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 stagger-reveal">
          {reportHighlights.map((item) => {
            ...
            return (
              <div
                key={item.label}
                className="rounded-2xl border border-border/30 bg-secondary/20 p-3 mini-stat-card"
              >
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2">
              {paymentBreakdown.map((p) => (
                <div
                  key={p.method}
                  className="flex items-center justify-between rounded-xl border border-border/30 px-3 py-2 row-interactive"
                >
            ...
            <Table>
              <TableHeader className="sticky top-0 z-10 bg-white/60 dark:bg-black/50 backdrop-blur-sm">
                <TableRow className="hover:bg-transparent border-black/5 dark:border-white/5">
                  <TableHead className="px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    Time
                  </TableHead>
                  ...
                  <TableHead className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    Amount
                  </TableHead>
                </TableRow>
              </TableHeader>
