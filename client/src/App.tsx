  ...
  return (
    ...
    >
      <span style={{ flex: 1, minWidth: 0 }}>
        ...
        {email ? (
          <> — We sent a verification link to <em>{email}</em>. Check your inbox to activate your account.</>
        ) : (
          <> — Check your inbox for the verification link.</>
        )}
      </span>
      {error && <span style={{ color: "#fca5a5", fontSize: "12px" }}>{error}</span>}
      {sent ? (
        <span style={{ color: "#a7f3d0", fontWeight: 600, fontSize: "12px" }}>✓ Email sent!</span>
      ) : (
        <button
