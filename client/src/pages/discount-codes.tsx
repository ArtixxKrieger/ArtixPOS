              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    type="number"
                    placeholder="0.00"
                    data-testid="input-discount-min-order"
                  />
                </FormControl>
              </FormItem>
            )}
...
export default function DiscountCodes() {
  const { data: codes = [], isLoading: _isLoading } = useQuery<DiscountCode[]>({
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (id: number) => {
      ...
      queryClient.setQueryData<any[]>(["/api/discount-codes"], (old) =>
        old ? old.filter((c) => c.id !== id) : [],
      );
      return { previous };
    },
    onError: (_e, _v, ctx) => {
      if (ctx?.previous) queryClient.setQueryData(["/api/discount-codes"], ctx.previous);
    },
    onSuccess: () => {
      undefined;
    },
  ...
  const toggleMutation = useMutation({
    ...
    onMutate: async ({ id, isActive }: { id: number; isActive: boolean }) => {
      ...
      const previous = queryClient.getQueryData<any[]>(["/api/discount-codes"]);
      queryClient.setQueryData<any[]>(["/api/discount-codes"], (old) =>
        old ? old.map((c) => (c.id === id ? { ...c, isActive } : c)) : [],
      );
      return { previous };
    },
    onError: (_e, _v, ctx) => {
      ...
      {codes.length === 0 ? (
        <div className="glass-card rounded-2xl py-16 text-center flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground/70">
            Create codes to offer discounts at checkout
          </p>
          ...
          <div className="divide-y divide-border/40">
            {codes.map((code) => {
              ...
              return (
                <div
                  key={code.id}
                  data-testid={`discount-code-row-${code.id}`}
                  className={[
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {code.type === "percentage"
                        ? `${code.value}% off`
                        : `${currency}${code.value} off`}
                      {code.minOrder && parseFloat(code.minOrder) > 0
                        ? ` · min ${currency}${code.minOrder}`
                        : ""}
                      {code.maxUses
                        ? ` · ${code.usedCount ?? 0}/${code.maxUses} used`
                        : ` · ${code.usedCount ?? 0} used`}
                  ...
                  <div className="flex items-center gap-0.5 shrink-0">
                    ...
                    <button
                      onClick={() =>
                        toggleMutation.mutate({ id: code.id, isActive: !code.isActive })
                      }
                      ...
                      aria-label={
                        code.isActive ? `Deactivate ${code.code}` : `Activate ${code.code}`
                      }
                    >
                      {code.isActive ? (
