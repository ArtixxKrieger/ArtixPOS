...
const REASON_LABELS: Record<string, string> = {
  ...
  redeem_discount: "Discount redeemed",
  redeem_product: "Free item redeemed",
  birthday: "Birthday bonus",
  referral: "Referral bonus",
  manual: "Manual adjustment",
  expiry: "Points expired",
  stamp_bonus: "Stamp card reward",
};
      ...
      <form onSubmit={form.handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="font-semibold text-sm">Birthday</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value ?? ""}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="font-semibold text-sm">Notes</FormLabel>
              <FormControl>
                <Textarea
                  {...field}
                  value={field.value ?? ""}
...
function CustomerProfile({
  ...
  const { data: rewards = [], isLoading: _rewardsLoading } = useQuery<LoyaltyReward[]>({
  ...
  const adjustPts = useMutation({
    mutationFn: ({ delta, note }: { delta: number; note?: string }) =>
      apiRequest("POST", `/api/customers/${customer.id}/loyalty`, {
        delta,
        note,
        reason: "manual",
      }).then((r) => r.json()),
  ...
  const redeemReward = useMutation({
    mutationFn: (rewardId: number) =>
      apiRequest("POST", `/api/customers/${customer.id}/redeem-reward`, { rewardId }).then((r) =>
        r.json(),
      ),
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: ["/api/customers"] });
    ...
    <div className="flex flex-col max-h-[88dvh]">
      ...
      <div className="p-5 bg-gradient-to-br from-primary/10 to-primary/5 border-b border-border/20 shrink-0">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold ${tier.bg} ${tier.color}`}
                >
          ...
          <button
            onClick={onEdit}
            className="h-8 w-8 rounded-xl hover:bg-muted/60 flex items-center justify-center text-muted-foreground"
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            <div
              key={s.label}
              className={`rounded-xl p-2 text-center ${s.highlight ? "bg-primary/20" : "bg-background/60"}`}
            >
              <p className={`text-sm font-black tabular-nums ${s.highlight ? "text-primary" : ""}`}>
                {s.value}
              </p>
            ...
            <div className="bg-card rounded-2xl border border-border/30 overflow-hidden">
              ...
              {sales.length === 0 ? (
                ...
                sales.slice(0, 5).map((s: any) => (
                  <div
                    key={s.id}
                    className="flex items-center justify-between px-3 py-2.5 border-t border-border/20"
                  >
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {Array.isArray(s.items) ? s.items.length : 0} item
                        {Array.isArray(s.items) && s.items.length !== 1 ? "s" : ""}
                      </p>
                    </div>
            ...
            <div className="bg-card rounded-2xl border border-border/30 p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  onClick={() =>
                    adjustPts.mutate({
                      delta: Math.abs(Number(manualDelta)),
          ...
          <>
            {pointsLog.length === 0 ? (
              ...
              pointsLog.map((log) => (
                <div
                  key={log.id}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-card border border-border/20 hover:bg-muted/20"
                  data-testid={`points-log-${log.id}`}
                >
                  <div
                    className={`h-8 w-8 rounded-xl flex items-center justify-center shrink-0 ${log.delta > 0 ? "bg-emerald-500/10" : "bg-rose-500/10"}`}
                  >
                    {log.delta > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-sm font-bold tabular-nums ${log.delta > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-500"}`}
                      >
                        {log.delta > 0 ? "+" : ""}
                        {log.delta} pts
                      </span>
                  ...
                  <p className="text-[10px] text-muted-foreground/40 shrink-0">
            ...
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl p-3 flex items-center gap-3">
              ...
              <div>
                <p className="text-base font-black text-primary">
                  {(customer.loyaltyPoints ?? 0).toLocaleString()} pts available
                </p>
            ...
            {activeRewards.length === 0 ? (
              ...
              activeRewards.map((r) => {
                ...
                return (
                  <div
                    key={r.id}
                    className={`bg-card rounded-2xl p-3 flex items-center gap-3 border transition-opacity ${canAfford ? "border-primary/20" : "border-border/20 opacity-50"}`}
                  ...
                  >
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <Button
                        size="sm"
                        className="mt-1 h-7 rounded-xl px-2.5 text-xs"
...
export default function Customers() {
  const { data: customers = [], isLoading: _isLoading } = useQuery<Customer[]>({
  ...
  const deleteMutation = useMutation({
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.previous) qc.setQueryData(["/api/customers"], ctx.previous);
    },
    onSuccess: () => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {c.tier && c.tier !== "none" && (
                        <span
                          className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[9px] font-semibold ${tier.bg} ${tier.color}`}
                        >
        ...
        <DialogContent className="max-w-lg rounded-3xl p-0 overflow-hidden">
          ...
          {profileCustomer && (
            <CustomerProfile
              ...
              onEdit={() => {
                setEditCustomer(profileCustomer);
                setProfileCustomer(null);
                setShowForm(true);
              }}
            />
