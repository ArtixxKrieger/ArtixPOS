...
export function registerTimeLogRoutes(app: Express): void {
  app.get("/api/time-logs", requireAuth, requirePro, async (req, res) => {
    ...
    // B-pattern: ?status=active returns current clock-in
    if (status === "active") {
      const log = await storage.getActiveTimeLog(getUserId(req));
      return res.json(log ?? null);
    }

    // B-pattern: ?scope=team returns team-wide time logs (manager only)
    if (scope === "team") {
      if (!["owner", "manager", "admin"].includes((req as any).user?.role ?? "")) {
