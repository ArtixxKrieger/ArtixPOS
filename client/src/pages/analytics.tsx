...
export default function Analytics() {
  ...
  const revPct = prevRevenue > 0 ? ((currRevenue - prevRevenue) / prevRevenue) * 100 : 0;
  ...
  const trendData = useMemo(() => {
    if (range.isDay) {
      ...
      currSales.forEach(s => { const h = new Date(s.createdAt!).getHours(); curr[h].revenue += rev(s); curr[h].orders++; });
      prevSales.forEach(s => { const h = new Date(s.createdAt!).getHours(); prev[h].revenue += rev(s); prev[h].orders++; });
      ...
      return days.map((day, i) => {
        ...
        return {
          ...
          revenue: cs.reduce((a, s) => a + rev(s), 0),
          orders: cs.length,
          prevRevenue: ps.reduce((a, s) => a + rev(s), 0),
  ...
  const productData = useMemo(() => {
    ...
    currSales.forEach(s => {
      ((s.items as any[]) || []).forEach(item => {
        ...
        const p = parseNumeric(item.size?.price ?? item.product?.price ?? 0);
  ...
  const insights = useMemo(() => {
    ...
    if (prevRevenue > 0) {
      const dir = currRevenue >= prevRevenue ? "up" : "down";
      list.push({
        icon: TrendingUp,
        text: `Revenue is ${dir} ${Math.abs(revPct).toFixed(1)}% compared to the previous period (${formatCurrency(prevRevenue, currency)} → ${formatCurrency(currRevenue, currency)}).`,
        color: currRevenue >= prevRevenue ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-rose-500/10 text-rose-600",
      });
    }
    if (productData[0]) {
      list.push({
        icon: Package,
        text: `Your best seller is "${productData[0].name}" with ${productData[0].qty} unit${productData[0].qty !== 1 ? "s" : ""} sold, generating ${formatCurrency(productData[0].revenue, currency)}.`,
    ...
    if (bestHour?.sales > 0) {
      list.push({
        icon: Clock,
        text: `Your peak hour is ${bestHour.label} with ${bestHour.sales} order${bestHour.sales !== 1 ? "s" : ""}. Consider extra staffing during this time.`,
    ...
    return list;
  }, [currRevenue, prevRevenue, revPct, productData, bestHour, dowData, categoryData, currency]);
      ...
      <div className="glass-card rounded-3xl overflow-hidden">
        <div className="px-5 py-4 border-b border-border/20 flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">
            {metric === "revenue" ? (showNet ? "Net Revenue" : "Gross Revenue") : "Order Volume"} — {preset === "today" ? "Hourly" : preset === "yesterday" ? "Hourly" : "Daily"}
          </span>
        </div>
        <div className="p-4 md:p-6">
          {trendData.every(d => (metric === "revenue" ? d.revenue : d.orders) === 0) ? (
            <div className="h-52 flex flex-col items-center justify-center text-muted-foreground/40 gap-2">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={trendData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                ...
                />
                {chartKind === "area" && (
                  <Area type="monotone" dataKey={metric} name="Current" stroke="#6366f1" strokeWidth={2.5} fill="url(#mainGrad)" isAnimationActive dot={false} />
                )}
                {chartKind === "bar" && (
                  <Bar dataKey={metric} name="Current" fill="#6366f1" radius={[4, 4, 0, 0]} isAnimationActive />
                )}
                {chartKind === "line" && (
                  <Line type="monotone" dataKey={metric} name="Current" stroke="#6366f1" strokeWidth={2.5} dot={false} isAnimationActive />
                )}
                {showComparison && (
                  <Line
                    type="monotone"
        ...
        <div className="glass-card rounded-3xl overflow-hidden">
          ...
          <div className="p-4">
            {hourlyData.every(d => d.sales === 0) ? (
              <div className="h-40 flex flex-col items-center justify-center text-muted-foreground/40 gap-2">
        ...
        <div className="glass-card rounded-3xl overflow-hidden">
          ...
          <div className="p-4">
            {dowData.every(d => d.revenue === 0) ? (
              <div className="h-40 flex items-center justify-center text-muted-foreground/40 text-sm">No data for period</div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="glass-card rounded-3xl overflow-hidden">
          ...
          <div className="p-5">
            {paymentData.length === 0 ? (
              <div className="h-40 flex items-center justify-center text-muted-foreground/40 text-sm">No payment data</div>
            ) : (
