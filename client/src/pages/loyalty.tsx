...
const PRESET_TIERS = [
  ...
  },
  {
    ...
    multiplier: "1.5",
    color: "#C0C0C0",
    perks: "1.5× points · Priority support",
    sortOrder: 2,
  },
  {
    name: "Gold",
    ...
    multiplier: "2",
    color: "#FFD700",
    perks: "2× points · Birthday bonus · Free delivery",
    sortOrder: 3,
  },
  {
    name: "Platinum",
    ...
    multiplier: "3",
    color: "#E5E4E2",
    perks: "3× points · VIP perks · Exclusive rewards",
    sortOrder: 4,
  },
];
...
function TierFormDialog({
  ...
  useEffect(() => {
    if (initial)
      form.reset({
        name: initial.name,
        minLifetimePoints: initial.minLifetimePoints,
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm rounded-3xl">
        <DialogHeader>
          <DialogTitle className="font-black">{initial ? "Edit Tier" : "New Tier"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(onSave)} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                type="number"
                min="0"
                {...form.register("minLifetimePoints", { valueAsNumber: true })}
                className="rounded-xl"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <Input
              {...form.register("perks")}
              placeholder="2× points · Birthday bonus"
              className="rounded-xl"
            />
          </div>
...
function RewardFormDialog({
  ...
  useEffect(() => {
    if (initial)
      form.reset({
        ...
        maxRedemptions: initial.maxRedemptions?.toString() ?? "",
  ...
  const valueLabel: Record<string, string> = {
    discount_fixed: "Discount Amount",
    discount_percent: "Discount Percent (%)",
    free_product: "Product ID",
    stamp_card: "Stamps required",
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-sm rounded-3xl max-h-[90dvh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-black">{initial ? "Edit Reward" : "New Reward"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(onSave)} className="space-y-3">
          <div>
...
export default function LoyaltyPage() {
  ...
  const [tierDialog, setTierDialog] = useState<{ open: boolean; tier?: LoyaltyTier | null }>({
    open: false,
  });
  ...
  const createTier = useMutation({
    mutationFn: (data: Partial<LoyaltyTier>) =>
      apiRequest("POST", "/api/loyalty/tiers", data).then((r) => r.json()),
    onSuccess: (result: LoyaltyTier) => {
      qc.setQueryData<LoyaltyTier[]>(["/api/loyalty/tiers"], (old) =>
  ...
  const updateTier = useMutation({
    mutationFn: ({ id, ...data }: Partial<LoyaltyTier> & { id: number }) =>
      apiRequest("PATCH", `/api/loyalty/tiers/${id}`, data).then((r) => r.json()),
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.previous) qc.setQueryData(["/api/loyalty/tiers"], ctx.previous);
    },
    onSuccess: (result: LoyaltyTier) => {
      qc.setQueryData<LoyaltyTier[]>(["/api/loyalty/tiers"], (old) =>
        old ? old.map((t) => (t.id === result.id ? result : t)) : [],
      );
      setTierDialog({ open: false });
    },
  });
  ...
  const createReward = useMutation({
    mutationFn: (data: Partial<LoyaltyReward>) =>
      apiRequest("POST", "/api/loyalty/rewards", data).then((r) => r.json()),
    onSuccess: (result: LoyaltyReward) => {
      qc.setQueryData<LoyaltyReward[]>(["/api/loyalty/rewards"], (old) =>
        old ? [...old, result] : [result],
      );
  ...
  const updateReward = useMutation({
    mutationFn: ({ id, ...data }: Partial<LoyaltyReward> & { id: number }) =>
      apiRequest("PATCH", `/api/loyalty/rewards/${id}`, data).then((r) => r.json()),
    ...
    onError: (_e, _v, ctx) => {
      if (ctx?.previous) qc.setQueryData(["/api/loyalty/rewards"], ctx.previous);
    },
    onSuccess: (result: LoyaltyReward) => {
      qc.setQueryData<LoyaltyReward[]>(["/api/loyalty/rewards"], (old) =>
        old ? old.map((r) => (r.id === result.id ? result : r)) : [],
      );
      setRewardDialog({ open: false });
    },
  });
  ...
  const toggleReward = useMutation({
    mutationFn: ({ id, isActive }: { id: number; isActive: boolean }) =>
      apiRequest("PATCH", `/api/loyalty/rewards/${id}`, { isActive }),
  ...
  const onSaveSettings = (data: Record<string, unknown>) => {
    updateSettings.mutate(data as any, { onSuccess: () => undefined });
  };
  ...
  const TYPE_LABEL: Record<string, string> = {
    discount_fixed: `${currency} Discount`,
    ...
    <div className="space-y-4 page-enter">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <div
            key={s.label}
            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-muted/50 border border-border/40"
          >
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatCard
              ...
              sub={`${stats.withPoints} with points`}
            />
            <StatCard
              icon={Star}
              label="Total Points"
              value={stats.totalPts.toLocaleString()}
            ...
            <StatCard
              icon={Gift}
              label="Redeemable"
              value={`${currency}${(stats.totalPts / Math.max(redemptionRate, 1)).toFixed(0)}`}
              sub="if all redeemed"
            ...
            <StatCard
              ...
              value={String(rewards.filter((r) => r.isActive).length)}
              sub={`${tiers.length} tiers`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      ...
                      <span className="text-xs tabular-nums text-muted-foreground">
                        {stats.tierCounts["none"] ?? customers.length}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div key={t.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span className="text-xs tabular-nums text-muted-foreground">
                          {stats.tierCounts[t.name.toLowerCase()] ?? 0}
                        </span>
                      </div>
                      <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${customers.length > 0 ? ((stats.tierCounts[t.name.toLowerCase()] ?? 0) / customers.length) * 100 : 0}%`,
          ...
          <div className="bg-card rounded-2xl border border-border/40 overflow-hidden shadow-sm">
            ...
            {stats.top.length === 0 ? (
              <div className="p-8 text-center text-sm text-muted-foreground">
                No customers with points yet. Attach customers to orders in POS to start tracking.
              </div>
            ) : (
              stats.top.map((c, idx) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="px-4 py-3 flex items-center gap-3 hover:bg-muted/20 border-t border-border/20 first:border-0"
                    data-testid={`row-loyalty-${c.id}`}
                  >
                    <div className="h-7 w-7 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-white flex items-center justify-center text-[11px] font-black shrink-0">
                      {idx + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {c.tier && c.tier !== "none" && (
                          <TierIcon className="h-3 w-3 text-amber-500 shrink-0" />
                        )}
                      </div>
                      <p className="text-[10px] text-muted-foreground">
                        {c.phone || c.email || `${c.visitCount ?? 0} visits`}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        ≈ {currency}
                        {((c.loyaltyPoints ?? 0) / Math.max(redemptionRate, 1)).toFixed(0)}
                      </p>
                    </div>
                  </div>
                );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Customers auto-progress based on lifetime points earned
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={() => setTierDialog({ open: true, tier: null })}
          ...
          {tiers.length === 0 ? (
            <div className="text-center py-12 bg-card rounded-2xl border border-border/40">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">
                Add tiers to reward loyal customers with better perks as they progress
              </p>
              <Button
                variant="outline"
                size="sm"
                ...
                onClick={seedPresetTiers}
              >
                Use Bronze/Silver/Gold/Platinum presets
              </Button>
            </div>
          ) : (
            tiers.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className="bg-card rounded-2xl border border-border/40 p-4 flex items-center gap-3 shadow-sm"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Unlocks at {t.minLifetimePoints.toLocaleString()} lifetime pts
                    </p>
                    {t.perks && (
                    ...
                    {deletingTier === t.id ? (
                      <>
                        <button
                          ...
                          className="h-8 w-8 rounded-xl bg-destructive/10 text-destructive flex items-center justify-center"
                        >
                          <Check className="h-3.5 w-3.5" />
                        </button>
                        <button
                          onClick={() => setDeletingTier(null)}
                          className="h-8 w-8 rounded-xl hover:bg-muted flex items-center justify-center text-muted-foreground"
                        >
                      ...
                      <>
                        ...
                        <button
                          onClick={() => setDeletingTier(t.id)}
                          className="h-8 w-8 rounded-xl hover:bg-destructive/10 hover:text-destructive flex items-center justify-center text-muted-foreground/50"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Customers redeem points for these rewards at checkout or from their profile
              </p>
            </div>
            <Button
              size="sm"
              className="rounded-xl gap-1.5"
              onClick={() => setRewardDialog({ open: true, reward: null })}
          ...
          {rewards.length === 0 ? (
            <div className="text-center py-12 bg-card rounded-2xl border border-border/40">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">
                Add fixed discounts, % discounts, free products, or custom rewards
              </p>
            </div>
          ) : (
            rewards.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className={`bg-card rounded-2xl border p-4 flex items-center gap-3 shadow-sm ${r.isActive ? "border-border/40" : "border-border/20 opacity-60"}`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-md ${r.isActive ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}
                      >
                        {r.isActive ? "Active" : "Inactive"}
                      </span>
                    </div>
                    <p className="text-[11px] text-muted-foreground">
                      {TYPE_LABEL[r.type]} · {r.pointsCost.toLocaleString()} pts
                    </p>
                    {r.description && (
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Switch
                      ...
                      onCheckedChange={(v) => toggleReward.mutate({ id: r.id, isActive: v })}
                    />
                    {deletingReward === r.id ? (
                      <>
                        <button
                      ...
                      <>
                        <button
                          onClick={() => setRewardDialog({ open: true, reward: r })}
                          className="h-8 w-8 rounded-xl hover:bg-muted flex items-center justify-center text-muted-foreground"
          ...
          <form onSubmit={settingsForm.handleSubmit(onSaveSettings)} className="space-y-4">
            ...
            <div className="bg-card rounded-2xl border border-border/40 p-4 space-y-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold mb-1.5 block">
                    Points per {currency}1 spent
                  </label>
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
                    {...settingsForm.register("loyaltyPointsPerUnit")}
                    className="rounded-xl h-9"
                    data-testid="input-points-per-unit"
                  />
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    e.g. 1 = 1 pt per {currency}1, 0.1 = 1 pt per {currency}10
                  </p>
                </div>
                <div>
                  <label className="text-xs font-semibold mb-1.5 block">
                    Points for {currency}1 off
                  </label>
                  <Input
                    type="number"
                    step="1"
                    min="1"
                    {...settingsForm.register("loyaltyRedemptionRate")}
                    className="rounded-xl h-9"
                    data-testid="input-redemption-rate"
                  />
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    e.g. 100 = 100 pts = {currency}1 discount
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl bg-violet-50 dark:bg-violet-950/20 border border-violet-200/40 dark:border-violet-500/20 p-3">
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div>
                    <p className="text-muted-foreground">{currency}1,000 spent</p>
                    <p className="font-bold">{Math.floor(1000 * pointsPerUnit)} pts</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Worth</p>
                    <p className="font-bold text-emerald-600 dark:text-emerald-400">
                      {currency}
                      {(Math.floor(1000 * pointsPerUnit) / Math.max(redemptionRate, 1)).toFixed(
                        2,
                      )}{" "}
                      off
                    </p>
                  ...
                  <div>
                    ...
                    <p className="font-bold">
                      {(
                        Math.floor(1000 * pointsPerUnit) /
                        Math.max(redemptionRate, 1) /
                        10
                      ).toFixed(1)}
                      % back
                    </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    type="number"
                    min="0"
                    {...settingsForm.register("loyaltyBirthdayBonus")}
                    className="rounded-xl h-9"
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    min="0"
                    {...settingsForm.register("loyaltyReferralBonus")}
                    className="rounded-xl h-9"
                    data-testid="input-referral-bonus"
                  />
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    Awarded to referrer when someone joins
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card rounded-2xl border border-border/40 p-4 space-y-3 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={settingsForm.watch("loyaltyStampEnabled")}
                  onCheckedChange={(v) => settingsForm.setValue("loyaltyStampEnabled", v)}
                  data-testid="switch-stamp-enabled"
                />
              </div>
              {settingsForm.watch("loyaltyStampEnabled") && (
                <div>
                  ...
                  <Input
                    ...
                    min="2"
                    max="50"
                    {...settingsForm.register("loyaltyStampTarget")}
                    className="rounded-xl h-9 w-32"
                    data-testid="input-stamp-target"
                  />
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    Each purchase = 1 stamp. Configure the stamp reward in the Rewards catalog above
                    (type: Stamp Card).
                  </p>
                </div>
              )}
            ...
            <div className="bg-card rounded-2xl border border-border/40 p-4 space-y-3 shadow-sm">
              ...
              <div>
                <label className="text-xs font-semibold mb-1.5 block">
                  Expire points after (days)
                </label>
                <Input
                  type="number"
                  min="0"
                  {...settingsForm.register("loyaltyExpiryDays")}
                  className="rounded-xl h-9 w-32"
