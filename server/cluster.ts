...
// Railway & other container platforms: use CLUSTER_WORKERS=1 to avoid
// forking workers inside a single container. Scale horizontally instead.
const isContainerPlatform =
  !!process.env.RAILWAY_ENVIRONMENT || !!process.env.RENDER || !!process.env.FLY_APP_NAME;
const DEFAULT_WORKERS = isContainerPlatform ? 1 : NUM_CPUS;
const NUM_WORKERS = WORKERS_ENV > 0 ? WORKERS_ENV : DEFAULT_WORKERS;
...
if (PRODUCTION && cluster.isPrimary && NUM_WORKERS > 1) {
  ...
  cluster.on("online", (w) => console.log(`[cluster] Worker ${w.process.pid} online`));
