...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  if (process.env.NODE_ENV !== "production") {
    app.get("/api/dev/email-preview/:type", async (req, res) => {
      const { buildVerificationEmailHtml, buildPasswordResetEmailHtml, buildReceiptEmailHtml } =
        await import("./email");
      ...
      if (type === "verify") {
        ...
        html = buildPasswordResetEmailHtml(
          "https://artixpos.com/reset-password?token=abc123preview",
        );
        ...
        html = buildReceiptEmailHtml(
          {
            ...
            items: [
              {
                product: { name: "Caramel Macchiato", price: "175.00" },
                size: { name: "Large", price: "195.00" },
                quantity: 1,
                modifiers: [{ name: "Extra Shot", price: "30.00" }],
              },
              {
                product: { name: "Cheesecake Slice", price: "165.00" },
                quantity: 1,
                modifiers: [],
              },
    ...
    app.post("/api/dev/email-send", async (req, res) => {
      ...
      if (!to || !type)
        return res.status(400).json({ ok: false, message: "body must contain { to, type }" });

      const { sendVerificationEmail, sendPasswordResetEmail, sendReceiptEmail, resetTransporter } =
        await import("./email");
      resetTransporter();

      let ok = false;
      if (type === "verify") {
        ok = await sendVerificationEmail(
          to,
          "https://artixpos.com/verify-email?token=TEST_TOKEN_PREVIEW",
        );
      } else if (type === "reset") {
        ok = await sendPasswordResetEmail(
          to,
          "https://artixpos.com/reset-password?token=TEST_TOKEN_PREVIEW",
        );
      } else if (type === "receipt") {
        ...
        return res
          .status(400)
          .json({ ok: false, message: "type must be: verify | reset | receipt" });
      }
      ...
      if (!ok) {
        return res
          ...
          .json({
            ok: false,
            message: "Send failed — check server logs. SMTP credentials may be missing or wrong.",
          });
      }
  ...
  // handlers work at both /api and /api/v1 without any code changes.
  app.use("/api/v1", (req, _res, next) => {
    req.url = "/api" + req.url; // /products → /api/products
    next();
  });
