...
import { Clock, Play, Trash2, CheckCircle2, XCircle } from "lucide-react";
...
export default function PendingOrders() {
  ...
  const handleComplete = (order: any) => {
    ...
    createSale.mutate({
      ...
      onSuccess: () => {
        ...
        toast({ title: "Order Completed", description: "Pending order has been processed as a sale." });
      }
    });
  };
  ...
  const handleUpdatePayment = (order: any) => {
    ...
    updateOrder.mutate({
      ...
      onSuccess: () => {
        toast({ title: "Payment Updated", description: "Order status has been updated." });
      }
    });
  };

  if (isLoading) return <div className="p-8 text-center animate-pulse">Loading pending orders...</div>;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage unpaid or parked transactions</p>
        </div>
      </div>
                    ...
                    <div className="space-y-3 mb-4">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          />
                          <Button size="sm" variant="outline" className="h-8 px-2 text-[10px]" onClick={() => handleUpdatePayment(order)}>
                            Update
                          </Button>
                        </div>
                      </div>
                      {Number(payments[order.id] || order.paymentAmount) > total && (
                        <div className="flex justify-between text-xs font-bold text-green-600">
                          <span>Change</span>
                          <span>{formatCurrency(Math.max(0, Number(payments[order.id] || order.paymentAmount) - total), settings?.currency)}</span>
                        </div>
                      )}
                    </div>
