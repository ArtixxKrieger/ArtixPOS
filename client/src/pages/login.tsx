...
function isPluginAvailable(name: string): boolean {
  try {
    return (window as any).Capacitor?.isPluginAvailable?.(name) === true;
  } catch {
    return false;
  }
}
...
function resolveOAuthUrl(path: string): string {
  ...
  if (base.startsWith("capacitor://") || base.startsWith("ionic://")) {
    throw new Error("VITE_API_BASE_URL is not configured. Please rebuild with a valid server URL.");
  }
...
async function openOAuthBrowser(provider: "google") {
  ...
  const url = resolveOAuthUrl(path);
...
function diagnoseNativeError(raw: string): string {
  ...
  if (msg.includes("10:") || msg.includes("developer_error") || msg.includes("something went wrong")) {
    return (
      "Google error 10 (DEVELOPER_ERROR): The app's signing fingerprint (SHA-1) " +
      "is not registered in Google Cloud Console."
    );
  }
...
async function nativeGoogleSignIn(): Promise<string> {
  ...
  debugLog("google", `webClientId baked in: ${webClientId ? webClientId.slice(0, 20) + "…" : "MISSING"}`);

  debugLog("google", "calling GoogleAuth.initialize…");
  ...
  debugLog("google", "calling GoogleAuth.signIn…");
  let googleUser: any;
  try {
...
export default function Login() {
  ...
  async function handleNativeGoogleSignIn() {
    setNativeError(null);
    setSigningIn(true);
    const pluginReady = isPluginAvailable("GoogleAuth");
    debugLog("login", `google tapped — plugin detected: ${pluginReady}`);
    sessionStorage.setItem(OAUTH_FLOW_KEY, "1");
    try {
      const token = await nativeGoogleSignIn();
      debugLog("login", "google sign-in OK — storing token");
      setNativeToken(token);
      await queryClient.invalidateQueries({ queryKey: ["auth-me"] });
      setLocation("/");
    } catch (err: any) {
      const msg: string = err?.message ?? String(err);
      debugLog("login", `google FAILED: ${msg}`);
      const isUserCancel =
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center gap-3 ${isDark ? "bg-[#080810]" : "bg-slate-50"}`}>
  ...
  return (
    ...
    >
      ...
      {/* Card */}
      <main
        className="rise relative z-10 w-full max-w-[420px] mx-5 rounded-3xl"
        style={{
          padding: "36px 32px",
      ...
      >
        ...
        <div className="rise d1 mb-6">
          ...
          <h1 style={{
            fontSize: 26, fontWeight: 800, lineHeight: 1.18,
          ...
          {(["signin", "register"] as AuthMode[]).map((m) => (
            <button
              ...
              style={{
                ...
                background: mode === m
                  ? isDark ? "rgba(109,40,217,0.5)" : "#ffffff"
                  : "transparent",
                color: mode === m
                  ? isDark ? "#ffffff" : "#6d28d9"
                  : isDark ? "rgba(255,255,255,0.45)" : "rgba(0,0,0,0.45)",
                boxShadow: mode === m
                  ? isDark ? "0 1px 4px rgba(0,0,0,0.4)" : "0 1px 4px rgba(0,0,0,0.1)"
                  : "none",
              }}
            >
        ...
        {reason === "banned" && (
          <div className="rise d1 mb-4" style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 14, color: isDark ? "#f87171" : "#b91c1c", marginBottom: 4 }}>
              Account Suspended
            </div>
        ...
        {hasPendingInvite && !error && !reason && (
          <div className="rise d1 mb-4" style={{
            padding: "11px 16px", borderRadius: 12, fontSize: 13, textAlign: "center",
            background: isDark ? "rgba(109,40,217,0.12)" : "rgba(109,40,217,0.07)",
        ...
        {/* Divider */}
        <div className="divider rise d2">
          <div className="divider-line" style={{ background: isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.09)" }} />
          <span style={{ fontSize: 12, fontWeight: 500, color: isDark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.35)", whiteSpace: "nowrap" }}>
        ...
        {/* Email form */}
        <form onSubmit={handleEmailSubmit} className="rise d3" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {mode === "register" && (
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: isDark ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.5)", display: "block", marginBottom: 5 }}>
                Full name
              </label>
              <input
                type="text"
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: isDark ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.5)", display: "block", marginBottom: 5 }}>
              Password {mode === "register" && <span style={{ fontWeight: 400, opacity: 0.7 }}>(min. 8 characters)</span>}
            </label>
        ...
        {/* Debug toggle — only shown on native (APK) builds */}
        {isNativePlatform() && (
      ...
      {showForgot && (
        <div style={{
          ...
          <div style={{
            width: "100%", maxWidth: 420, borderRadius: 24, padding: "36px 32px",
            background: isDark ? "#0f0c1a" : "#ffffff",
            ...
            {forgotSuccess ? (
              <div style={{ textAlign: "center" }}>
                ...
                <h2 style={{ fontSize: 20, fontWeight: 700, margin: "0 0 8px", color: isDark ? "#fff" : "#0f0a1e" }}>
                  Check your email
                </h2>
              ...
              <>
                ...
                <h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 6px", color: isDark ? "#fff" : "#0f0a1e" }}>
                  Reset password
                </h2>
                <p style={{ fontSize: 13, lineHeight: 1.6, color: isDark ? "rgba(255,255,255,0.55)" : "rgba(15,10,30,0.6)", margin: "0 0 24px" }}>
      ...
      {isNativePlatform() && showDebug && (
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ color: "#a78bfa", fontWeight: 700, fontSize: 12, letterSpacing: "0.05em" }}>
              ARTIXPOS DEBUG
            </span>
            <div style={{ display: "flex", gap: 6 }}>
              <button
                ...
                style={{
                  ...
                  border: "1px solid rgba(96,165,250,0.2)", borderRadius: 5,
              ...
              >Refresh</button>
              <button
                onClick={() => { clearDebugLogs(); setDebugEntries([]); }}
                style={{
          ...
          <div style={{
            ...
            {[
              { label: "API", value: API_BASE ? API_BASE.replace("https://", "") : "(not set)", ok: !!API_BASE },
              { label: "Google plugin", value: isPluginAvailable("GoogleAuth") ? "READY" : "NOT SYNCED", ok: isPluginAvailable("GoogleAuth") },
              { label: "Auth", value: isLoading ? "loading…" : isAuthenticated ? "authenticated" : "not auth", ok: isAuthenticated },
            ].map(({ label, value, ok }) => (
              <div key={label} style={{ display: "flex", gap: 4, alignItems: "center" }}>
                <span style={{ color: ok ? "#4ade80" : "#f87171", fontSize: 9 }}>●</span>
                <span style={{ color: "#64748b" }}>{label}:</span>
                <span style={{ color: ok ? "#4ade80" : "#f87171", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 100 }}>{value}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ overflowY: "auto", flex: 1, padding: "4px 0" }}>
            {debugEntries.length === 0 ? (
              <div style={{ color: "#475569", fontStyle: "italic", padding: "8px 12px" }}>
                No logs yet — tap a sign-in button.
              </div>
            ) : (
              [...debugEntries].reverse().map((e, i) => {
                const isFail = e.msg.toLowerCase().includes("fail") || e.msg.toLowerCase().includes("error") || e.msg.toLowerCase().includes("threw");
                const isOk = e.msg.toLowerCase().includes("ok ✓") || e.msg.toLowerCase().includes("ready");
                return (
                  <div key={i} style={{
                    padding: "3px 12px",
                    borderLeft: isFail ? "2px solid #f87171" : isOk ? "2px solid #4ade80" : "2px solid transparent",
                    background: isFail ? "rgba(248,113,113,0.05)" : "transparent",
                  }}>
