...
export async function getLoyaltyTiers(userId: string): Promise<LoyaltyTier[]> {
  ...
  const condition =
    userIds.length === 1
      ? eq(loyaltyTiers.userId, userIds[0])
      : inArray(loyaltyTiers.userId, userIds);
...
export async function updateLoyaltyTier(
  id: number,
  userId: string,
  tier: Partial<InsertLoyaltyTier>,
): Promise<LoyaltyTier | undefined> {
  const userIds = await getTenantUserIds(userId);
  const condition =
    userIds.length === 1
      ? and(eq(loyaltyTiers.id, id), eq(loyaltyTiers.userId, userIds[0]))
      : and(eq(loyaltyTiers.id, id), inArray(loyaltyTiers.userId, userIds));
...
export async function getLoyaltyRewards(userId: string): Promise<LoyaltyReward[]> {
  ...
  const condition =
    userIds.length === 1
      ? and(eq(loyaltyRewards.userId, userIds[0]), isNull(loyaltyRewards.deletedAt))
      : and(inArray(loyaltyRewards.userId, userIds), isNull(loyaltyRewards.deletedAt));
...
export async function updateLoyaltyReward(
  ...
  const condition =
    userIds.length === 1
      ? and(eq(loyaltyRewards.id, id), eq(loyaltyRewards.userId, userIds[0]))
      : and(eq(loyaltyRewards.id, id), inArray(loyaltyRewards.userId, userIds));
 ...
 *
 * All three mutations (deduct customer points, increment reward redemption count,
 * insert the points log) run inside a single DB transaction. Previously these were
 * three separate statements — if the reward counter or log insert failed, points
 * were silently deducted without any record.
 */
export async function redeemLoyaltyReward(
  ...
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      await tx
        .update(loyaltyRewards)
        .set({ redemptionCount: sql`COALESCE(redemption_count, 0) + 1` } as any)
...
export async function getLoyaltyPointsLog(customerId: number, userId: string): Promise<LoyaltyPointsLog[]> {
  try {
    ...
    return db
      .select()
      .from(loyaltyPointsLog)
      .where(eq(loyaltyPointsLog.customerId, customerId))
      .orderBy(desc(loyaltyPointsLog.createdAt))
      .limit(100);
  } catch {
    return [];
  }
...
export async function addLoyaltyPointsLog(
  userId: string,
  customerId: number,
  delta: number,
  reason: string,
